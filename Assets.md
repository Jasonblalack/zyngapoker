...
Both methods offer batch loading approaches. Preloading is different than load here though. Load just loads the data and returns it (still applies CSS files to the document), while preloading keeps results cached for `getImage()`/`getFrame()` methods. Both methods support loading all file types supported by `core.io.Queue`:

* `js`: JavaScript files `core.io.Script`
* `css`: CSS files `core.io.StyleSheet`
* `jsonp`: JSON files `core.io.Jsonp`
* `json`: JSON files `core.io.Text`
* `txt`: Text files `core.io.Text`
* `md`: Markdown files `core.io.Text`
* `html`: HTML files `core.io.Text`
* `png`: PNG images via `core.io.Image`
* `jpeg`: JPEG images via `core.io.Image`
* `jpg`: JPEG images via `core.io.Image`
* `gif`: GIF images via `core.io.Image`

**Note:** `core.io.Queue` supports parallel loading when the loaders supports it (depends on file type and browser). It keeps order of files when using `core.io.Asset.load()`/`core.io.Queue.load()` so that JavaScript files which extend each other can safely be batch loaded with that class.


### Querying Image Details

All these methods works with pure meta data being generated by Jasy. You can directly access the DOM node of images as well (e.g. for Canvas based drawing), if the section was preloaded before using `core.io.Asset.preloadSection()`
